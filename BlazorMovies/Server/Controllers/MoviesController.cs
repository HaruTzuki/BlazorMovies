...
namespace BlazorMovies.Server.Controllers
{
    ...
    public class MoviesController : ControllerBase
    {
        ...
        public async Task<ActionResult<IndexPageDTO>> Get()
        {
            ...
            var upcomingRelease = await context.Movies.Where(x => x.ReleaseDate > todayDate).OrderBy(x => x.ReleaseDate).Take(limit).ToListAsync();

            var response = new IndexPageDTO();
            response.InTheaters = moviesInTheaters;
            response.UpcomingRelease = upcomingRelease;
        ...
        public async Task<ActionResult> Put(Movie movie)
        {
            ...
            if (!string.IsNullOrWhiteSpace(movie.Poster))
            {
                ...
                movieDB.Poster = await fileStorageService.EditFile(moviePoster, "jpg", "movies", movieDB.Poster);
            }
        ...
        {
            var movie = await context.People.FirstOrDefaultAsync(x => x.Id == Id);
